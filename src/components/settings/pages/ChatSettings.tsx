...
export function ChatSettings({ settings, onChange, saving }: Props) {
  ...
  return <div className="mx-auto max-w-[760px] px-10 py-12">
    <header className="mb-9">
      <h1 className="flex items-center gap-2 text-2xl font-medium text-foreground"><MessageSquareText size={21}/> Chat</h1>
      <p className="mt-2 text-sm text-foreground/50">Composer and transcript controls apply immediately to Mission Control.</p>
    </header>

    <section className="mb-8">
      <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-foreground"><Keyboard size={15}/> Composer</div>
    ...
    <div className="rounded-xl border border-border bg-white/[0.02] p-5">
      <h2 className="flex items-center gap-2 text-sm font-medium text-foreground"><History size={14}/> Local chat data</h2>
      <p className="mt-2 text-[13px] leading-relaxed text-foreground/45">Chat history is stored locally and never grants workspace tools. Mission Control task transcripts continue to use the durable task ledger.</p>
      ...
      {historyMessage && <p className="mt-3 text-xs text-foreground/50" role="status">{historyMessage}</p>}
    </div>

    <p className="mt-5 flex items-center gap-2 text-xs text-foreground/40"><ShieldCheck size={13}/>{saving ? "Saving chat preferences…" : "Composer preferences are stored locally."}</p>
  </div>;
}
