...
export function ComputerUseSettings({ settings, onChange, saving }: Props) {
  ...
  return <div className="max-w-[700px] mx-auto px-10 py-12">
    <h1 className="flex items-center gap-2 text-2xl font-medium text-foreground"><Monitor size={21}/> Computer use</h1>
    <p className="mt-2 mb-8 text-sm text-foreground/50">Controls model-facing desktop actions and explicit context capture. Direct Open ChatGPT handoffs remain visible, user-invoked actions.</p>
    ...
    <p className="mt-5 flex items-center gap-2 text-xs text-foreground/40"><ShieldCheck size={13}/>{saving ? "Saving privacy boundary…" : "Background desktop actions are not exposed until a resumable approval flow exists."}</p>
  </div>;
}
