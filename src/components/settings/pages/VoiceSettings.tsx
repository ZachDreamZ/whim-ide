...
export function VoiceSettings({ settings, onChange, saving }: Props) {
  ...
  return <div className="max-w-[700px] mx-auto px-10 py-12">
    ...
    <label htmlFor="whim-wake-phrase" className="mt-8 mb-3 block text-sm font-semibold text-[#ececf1]">Wake phrase (optional)</label>
    <p className="mb-3 text-[13px] leading-relaxed text-white/45">When set, ambient mode only forwards an utterance after it hears this phrase, and strips the phrase before sending. Leave blank to forward everything you say.</p>
    <input id="whim-wake-phrase" value={settings.voice.wakePhrase} maxLength={60} onChange={(event) => update({ wakePhrase: event.target.value })} placeholder="Hey Whim" className="w-full rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-[#ececf1] outline-none placeholder:text-white/25 focus:border-white/25" />
