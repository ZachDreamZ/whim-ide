...
export function DeploymentModal({ workspace, onClose }: Props) {
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      const result = await bridge.deploy(workspace, "vercel", isProduction);
      if (result.success) {
        setStatus("success");
        setMessage("Deployment successful!");
        setUrl(result.stdout || "https://whim-deployed-app.vercel.app");
      } else {
        setStatus("error");
          ...
          {status === "idle" && (
            <>
              ...
              <button 
                ...
                className="w-full flex justify-center items-center gap-2 px-4 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium transition-colors"
              >
