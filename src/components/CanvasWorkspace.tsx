...
import { Code2, Globe, LoaderCircle, Redo2, RefreshCw, Save, Undo2, X } from "lucide-react";
...
export function CanvasWorkspace({ workspace, entries, initialPath, onClose, onSaved }: Props) {
  ...
  const [view, setView] = useState<"code" | "preview">("code"); const [busy, setBusy] = useState(false); const [message, setMessage] = useState("");
  ...
  return <section className="flex flex-col w-full h-full bg-[#1e1e1e] rounded-xl border border-white/10 overflow-hidden shadow-2xl">
    <header className="flex items-center gap-2 px-3 py-2 bg-[#2d2d2d] border-b border-white/5"><select aria-label="Canvas file" value={path} onChange={(event) => selectPath(event.target.value)} className="min-w-0 flex-1 bg-black/25 text-xs text-white rounded px-2 py-1.5 border border-white/10">{files.map((file) => <option key={file.path} value={file.path}>{file.path}</option>)}</select>
      <button onClick={() => setView("code")} className={`p-1.5 rounded ${view === "code" ? "bg-white/15 text-white" : "text-white/50"}`} title="Code"><Code2 size={14}/></button><button onClick={() => setView("preview")} className={`p-1.5 rounded ${view === "preview" ? "bg-white/15 text-white" : "text-white/50"}`} title="Preview"><Globe size={14}/></button>
    ...
    {view === "code" ? <textarea aria-label="Canvas editor" value={document.content} onChange={(event) => dispatch({ type: "edit", content: event.target.value })} spellCheck={false} className="flex-1 resize-none bg-[#1a1a1a] text-[#d4d4d4] p-4 font-mono text-[13px] leading-relaxed outline-none" /> : <iframe title="Canvas preview" sandbox={/\.html?$/i.test(path) ? "allow-scripts" : ""} srcDoc={preview} className="flex-1 w-full bg-white" />}
  </section>;
}
