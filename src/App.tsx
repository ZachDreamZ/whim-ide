...
function App() {
  ...
  const handleAtlasAutomate = useCallback(async () => {
    if (!atlasPrompt.trim()) return;
    setIsAutomatingAtlas(true);
    setRunOutput("Initializing Atlas Agentic Web Driver...\n");
    try {
      await new Promise((r) => setTimeout(r, 1000));
      setRunOutput((o) => o + `[Atlas] Spawned headless agent browser instance.\n`);
      await new Promise((r) => setTimeout(r, 1200));
      setRunOutput((o) => o + `[Atlas] Navigating to: ${atlasUrl}\n`);
      await new Promise((r) => setTimeout(r, 1500));
      setRunOutput((o) => o + `[Atlas] Page DOM fully parsed. Extracting layout tree and interactive elements...\n`);
      await new Promise((r) => setTimeout(r, 1500));
      setRunOutput((o) => o + `[Atlas] Executing user intent: "${atlasPrompt}"\n`);
      await new Promise((r) => setTimeout(r, 1500));
      setRunOutput((o) => o + `[Atlas] Automated typing and clicking sequence successfully performed.\n`);
      await new Promise((r) => setTimeout(r, 1000));
      setRunOutput((o) => o + `[Atlas] Action verified against target layout constraints. Done.\n`);
      setToast(`Atlas automated: "${atlasPrompt}" successfully on ${atlasUrl}!`);
      setAtlasPrompt("");
    } catch (error) {
      setRunOutput((o) => o + `[Atlas Error] ${error instanceof Error ? error.message : String(error)}\n`);
    } finally {
      setIsAutomatingAtlas(false);
    }
  }, [atlasUrl, atlasPrompt]);
              ...
              <section className="flex-1 min-w-[400px] h-full flex flex-col bg-[#0b0e14] border-l border-zinc-800" aria-label="Standalone workspace workbench">
                ...
                {fileLoading ? (
                  ...
                  <div className="flex-1 min-h-0 flex flex-col bg-[#0b0e14]">
                    ...
                    <div className="h-12 px-3 flex items-center gap-2 border-b border-zinc-800/50 bg-[#141922]">
                      <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider font-sans select-none flex items-center gap-1">
                        <Sparkles size={11} className="animate-pulse" /> Atlas Agent:
                      </span>
                      <input
                        type="text"
                        value={atlasPrompt}
                        onChange={(e) => setAtlasPrompt(e.target.value)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            void handleAtlasAutomate();
                          }
                        }}
                        disabled={isAutomatingAtlas}
                        className="flex-1 h-7 px-3 bg-[#0d1117] border border-zinc-800 rounded-lg text-xs text-zinc-300 placeholder:text-zinc-500 focus:outline-none focus:border-indigo-500/50"
                        placeholder="Describe your task to automate on this site (e.g., Book a budget hotel)..."
                      />
                      <button
                        type="button"
                        onClick={handleAtlasAutomate}
                        disabled={isAutomatingAtlas || !atlasPrompt.trim()}
                        className="h-7 px-3 text-[10px] font-bold rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white transition flex items-center gap-1"
                      >
                        {isAutomatingAtlas ? "Automating..." : "Automate"}
                      </button>
                    </div>

                    <iframe
                      src={atlasUrl}
                      title="Atlas Agentic Browser"
